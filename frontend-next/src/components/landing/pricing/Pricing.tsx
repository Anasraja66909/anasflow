...
const Pricing = () => {
  ...
  return (
    <section
      id="pricing"
      className="py-16 md:py-32 bg-black relative overflow-hidden border-t border-white/5"
    >
      {/* Immersive Background Architecture */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[300px] md:w-[1200px] h-[300px] md:h-[600px] bg-indigo-500/5 blur-[100px] md:blur-[180px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-screen-2xl mx-auto px-6 md:px-8 relative z-10">
        ...
        >
          ...
          >
            ...
            <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.4em] md:tracking-[0.5em] text-indigo-400/60">
              Simple Pricing
            </span>
          </motion.div>
          ...
          <motion.h2
            variants={itemVariants}
            className="text-4xl sm:text-6xl lg:text-7xl xl:text-[90px] font-black tracking-tighter text-white leading-[1] md:leading-[0.85] uppercase break-words"
          >
          ...
          <motion.p
            variants={itemVariants}
            className="text-lg md:text-2xl text-zinc-600 max-w-3xl mx-auto font-medium leading-relaxed italic"
          >
        ...
        >
          <div className="flex items-center gap-2 bg-zinc-950/40 backdrop-blur-3xl border border-white/5 p-1.5 sm:p-2 rounded-full shadow-2xl overflow-hidden">
            <button
              onClick={() => setBillingCycle("monthly")}
              className={`px-6 sm:px-10 py-3 sm:py-5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.4em] transition-all duration-700 ${billingCycle === "monthly" ? "bg-white text-black shadow-3xl" : "text-zinc-600 hover:text-white"}`}
            >
              Monthly
            </button>
            <button
              onClick={() => setBillingCycle("yearly")}
              className={`px-6 sm:px-10 py-3 sm:py-5 rounded-full text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.4em] transition-all duration-700 ${billingCycle === "yearly" ? "bg-white text-black shadow-3xl" : "text-zinc-600 hover:text-white"}`}
            >
              Yearly{" "}
        ...
        >
          {PLANS.map((plan) => {
            ...
            return (
              ...
              >
                {plan.badge && (
                  <div className="absolute top-8 md:top-10 right-8 md:right-10 px-4 sm:px-5 py-2 bg-[#00E5C0] text-black text-[8px] sm:text-[9px] font-black uppercase tracking-widest rounded-full shadow-[0_0_20px_rgba(0,229,192,0.4)]">
                    {plan.badge}
                  </div>
                )}
                ...
                <div className="text-left space-y-6">
                  <h3
                    className={`text-4xl md:text-5xl font-black uppercase tracking-tighter leading-none ${plan.accent || "text-white"}`}
                  >
                    {plan.name}.
                  </h3>
                  <p className="text-zinc-600 text-base md:text-lg font-medium leading-relaxed italic pr-12">
                    {plan.desc}
                  </p>
                  ...
                  <div className="pt-8 md:pt-10 pb-10 md:pb-12 flex items-baseline gap-3">
                    ...
                    <span className="text-zinc-800 font-black uppercase tracking-[0.4em] text-[9px] md:text-[10px]">
                      /{billingCycle === "yearly" ? "ANN" : "MO"}
                    </span>
                  </div>

                  <div className="space-y-4 md:space-y-6 pb-10 md:pb-12 border-b border-white/5">
                    {plan.features.map((f) => (
                      <div
                        key={f}
                        className="flex items-center gap-4 sm:gap-5 text-zinc-500 group-hover:text-zinc-300 transition-colors duration-700"
                      >
                    ...
                    {plan.missing.map((f) => (
                      <div
                        key={f}
                        className="flex items-center gap-4 sm:gap-5 text-zinc-800"
                      >
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={{ once: true }}
          className="mt-16 md:mt-24 p-8 md:p-12 bg-zinc-950/40 backdrop-blur-[60px] border border-white/5 rounded-[3rem] md:rounded-[4rem] flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 max-w-5xl mx-auto shadow-3xl text-center md:text-left"
        >
          <div className="space-y-3">
            <h4 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter leading-none">
              Enterprise Plan.
            </h4>
            <p className="text-zinc-600 text-sm md:text-base font-medium italic">
              Custom features for large teams and dedicated support.
            </p>
          </div>
          <button className="w-full md:w-auto px-10 md:px-12 py-5 md:py-6 bg-zinc-900 border border-white/5 text-zinc-400 hover:text-white rounded-[1.5rem] md:rounded-[1.75rem] text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] transition-all flex items-center justify-center gap-4 sm:gap-6 group shadow-xl">
            Contact Us
