...
// Slot component for asChild prop support
    ...
    return (
      <SidebarContext.Provider
        value={{
          state,
          open: state === "expanded",
          setOpen: (open) => setState(open ? "expanded" : "collapsed"),
          openMobile,
          setOpenMobile,
          isMobile,
          toggleSidebar,
      ...
      <nav className="flex-1 py-4">
        {navItems.map((item) => (
          <Link
            key={item.href}
            href={item.href}
            className={`block px-6 py-3 rounded-lg mb-1 transition-colors font-medium ${pathname === item.href ? "bg-primary text-primary-foreground" : "hover:bg-muted"}`}
          >
            {item.label}
          </Link>
        ))}
