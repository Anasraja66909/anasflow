  ...
  return (
    <div className="min-h-screen text-white font-sans pb-20 w-full overflow-x-hidden relative">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-8 mb-24">
          ...
          <KpiCard
            ...
            subtext={{ highlight: "", normal: "potential monthly optimization" }}
            icon={Activity}
            loading={loading}
              ...
              >
                ...
                <div className="flex justify-between items-center mb-16">
                  <div className="space-y-1">
                    ...
                    <p className="text-[9px] text-slate-400 dark:text-zinc-600 font-bold tracking-[0.2em] uppercase ml-0.5">
                ...
                <div className="h-[400px] w-full min-w-0">
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <XAxis dataKey="name" stroke="#52525b" fontSize={9} fontWeight="900" tickLine={false} axisLine={false} dy={15} />
                      <YAxis stroke="#52525b" fontSize={9} fontWeight="900" tickLine={false} axisLine={false} dx={-15} tickFormatter={(value) => `$${value}`} />
                      <RechartsTooltip
                        contentStyle={{
                ...
                <div className="flex flex-col md:flex-row items-center gap-10 md:gap-16">
                  <div className="w-full max-w-[280px] sm:max-w-[300px] aspect-square shrink-0 relative">
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                      <span className="text-[10px] font-black text-zinc-700 uppercase tracking-widest">Density</span>
                      <span className="text-3xl md:text-4xl font-black text-white">360°</span>
                    </div>
                    <ResponsiveContainer width="100%" height="100%">
                      <PieChart>
                        <Pie data={formattedBreakdown} cx="50%" cy="50%" innerRadius={100} outerRadius={135} paddingAngle={6} dataKey="cost" stroke="none">
                  ...
                  <div className="flex-1 w-full space-y-8">
                    <div className="space-y-1">
                      <h3 className="text-2xl font-black tracking-tighter text-white uppercase">Cost Breakdown</h3>
                      <p className="text-[10px] text-zinc-600 font-black tracking-widest uppercase ml-0.5">Cost by platform</p>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6">
                      {formattedBreakdown.map((item: any, idx: number) => (
                        <div key={idx} className="flex flex-col gap-3 p-6 rounded-[1.8rem] bg-slate-50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 hover:border-[#00E5C0]/30 transition-all duration-500 group/item cursor-pointer shadow-sm dark:shadow-none">
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex items-center gap-3 min-w-0">
                              <div className="w-2.5 h-2.5 rounded-full shrink-0 shadow-[0_0_8px_rgba(0,0,0,0.1)]" style={{ backgroundColor: item.color }}></div>
                              <span className="font-bold uppercase text-[9px] tracking-widest text-slate-500 dark:text-zinc-500 group-hover/item:text-slate-900 dark:group-hover/item:text-zinc-300 transition-colors truncate">{item.name}</span>
                            </div>
                            <span className="text-[9px] font-black text-slate-400 dark:text-zinc-700 shrink-0">{item.value}%</span>
                          </div>
                          <span className="text-xl font-extrabold text-slate-900 dark:text-white tracking-tight whitespace-nowrap">${item.cost.toLocaleString()}</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="lg:col-span-5 xl:col-span-4 flex flex-col gap-10 w-full relative">
              <motion.div variants={itemVariants} className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {data?.health_status?.map((item: any) => (
                  <div key={item.id} className="group bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-[3rem] p-8 shadow-2xl relative hover:border-white/20 transition-all duration-700 cursor-pointer overflow-hidden min-h-[160px] flex flex-col justify-between">
                    <div className="absolute top-0 right-0 w-32 h-32 blur-[80px] rounded-full opacity-20 pointer-events-none transition-opacity duration-700 group-hover:opacity-40" style={{ backgroundColor: item.color }}></div>
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-1.5 h-1.5 rounded-full animate-ping" style={{ backgroundColor: item.color }} />
                        <span className="text-[10px] font-black uppercase tracking-[0.2em]" style={{ color: item.color }}>{item.status}</span>
                      </div>
                    </div>
                    <div>
                      <h4 className="font-black text-white tracking-widest text-[11px] uppercase mb-1">{item.name}</h4>
                      <p className="text-[10px] text-zinc-700 font-bold tracking-widest uppercase truncate">{item.time}</p>
                    </div>
                  </div>
                ))}
              </motion.div>

              <motion.div variants={itemVariants} className="bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-[3.5rem] p-12 shadow-2xl flex-1 flex flex-col relative overflow-hidden group">
                ...
                <div className="flex items-center gap-4 mb-12">
                  ...
                  <div className="space-y-0.5 text-left">
                    <h3 className="text-2xl font-black text-white tracking-tighter uppercase leading-none">Suggestions</h3>
                    <p className="text-[10px] text-zinc-600 font-black uppercase tracking-[0.3em] ml-0.5">Cost Optimization</p>
                  </div>
                </div>
                <div className="flex flex-col gap-6 mb-auto text-left">
                  <div className="bg-white/[0.02] border border-white/5 p-8 rounded-[2.5rem] hover:bg-white/[0.05] transition-all duration-500 relative group/card border-l-4 border-l-[#00E5C0]">
                    <p className="text-[13px] mb-6 text-zinc-400 font-medium leading-relaxed">
                      Detected signal variance on your <span className="font-black text-white">active nodes</span>. Synchronizing layer-2 optimization to prune redundant latency.
                    </p>
                    <p className="text-[#00E5C0] text-[11px] font-black uppercase tracking-[0.3em] flex items-center gap-3">
                      <span className="w-2 h-2 bg-[#00E5C0] rounded-full animate-ping shadow-[0_0_10px_rgba(0,229,192,0.5)]" /> Real-time optimization enabled
                    </p>
                  </div>
                </div>
                <button className="w-full py-6 mt-12 rounded-[1.5rem] font-black text-[11px] uppercase tracking-[0.3em] text-[#00E5C0] border border-[#00E5C0]/20 hover:bg-[#00E5C0] hover:text-black transition-all duration-700 flex items-center justify-center gap-4 group/btn">
                  View All Suggestions <ArrowRight className="w-4 h-4 translate-x-0 group-hover/btn:translate-x-3 transition-transform" />
                </button>
              </motion.div>

              <motion.div variants={itemVariants} className="bg-gradient-to-br from-[#00E5C0] via-[#00E5C0] to-teal-600 rounded-[3rem] p-12 shadow-2xl mt-auto relative overflow-hidden group cursor-pointer hover:scale-[1.02] transition-transform duration-700">
                <div className="absolute -top-10 -right-10 w-48 h-48 bg-white/30 blur-[60px] rounded-full animate-pulse" />
                <div className="relative z-10 text-left">
                  <h3 className="font-black text-3xl text-black tracking-tighter mb-4">Investment Report<span className="text-white">.</span></h3>
                  <p className="text-sm text-black/60 font-semibold mb-10 leading-relaxed max-w-[240px]">Export unified value metrics and ROI visualizations for account nodes.</p>
        ...
        {hasPlatforms && (
          <motion.div variants={itemVariants} className="relative z-10 bg-zinc-950/40 backdrop-blur-3xl border border-white/5 rounded-[4rem] shadow-2xl overflow-hidden w-full group">
            <div className="p-12 md:p-16 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center bg-white/[0.01] gap-8">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-3xl font-black tracking-tighter text-white uppercase">Recent Activity</h3>
                </div>
                <p className="text-[11px] text-zinc-600 font-black tracking-[0.4em] uppercase ml-11">Recent platform events</p>
              </div>
              <button className="text-[11px] font-black uppercase tracking-[0.4em] text-[#00E5C0] hover:text-white flex items-center gap-4 transition-all group/log border border-[#00E5C0]/20 px-8 py-4 rounded-full hover:bg-[#00E5C0]/10">
                View All Activity <ArrowRight className="w-4 h-4 group-hover/log:translate-x-3 transition-transform" />
              </button>
            </div>
              ...
              <table className="w-full text-left border-collapse min-w-[900px]">
                <thead>
                  <tr className="bg-zinc-950/20 border-b border-white/5">
                    <th className="px-12 py-8 text-[11px] font-black text-zinc-500 uppercase tracking-[0.4em]">Platform</th>
                    <th className="px-12 py-8 text-[11px] font-black text-zinc-500 uppercase tracking-[0.4em]">Activity</th>
                    <th className="px-12 py-8 text-[11px] font-black text-zinc-500 uppercase tracking-[0.4em] text-right">Cost</th>
                    <th className="px-12 py-8 text-[11px] font-black text-zinc-500 uppercase tracking-[0.4em] text-right">Time</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/5">
                  {(data?.recent_activity || []).length > 0 ? (
                      ...
                      <tr key={log.id} className="group/row hover:bg-white/[0.02] transition-colors">
                        <td className="px-12 py-10 whitespace-nowrap">
                          <div className="flex items-center gap-6">
                            ...
                            <span className="font-black text-sm uppercase tracking-[0.1em] text-white">{log.platform}</span>
                          </div>
                        </td>
                        <td className="px-12 py-10 whitespace-nowrap text-xs font-semibold text-zinc-400 group-hover/row:text-zinc-200 transition-colors uppercase tracking-widest">{log.activity}</td>
                        <td className="px-12 py-10 whitespace-nowrap text-right">
                          <span className="inline-block px-6 py-2 rounded-full bg-white/5 border border-white/5 text-xs font-black text-white group-hover/row:border-[#00E5C0]/40 transition-all shadow-inner">{log.cost}</span>
                        </td>
                        <td className="px-12 py-10 whitespace-nowrap text-right">
                    ...
                    <tr>
                      <td colSpan={4} className="px-12 py-20 text-center text-[10px] text-zinc-600 font-black uppercase tracking-widest">
                        Waiting for platform handshake signals...
                      </td>
                    </tr>
                  )}
                </tbody>
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb:hover { background: rgba(0, 229, 192, 0.2); }
      `}</style>
    </div>
  );
}
